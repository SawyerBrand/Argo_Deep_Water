...
title('Float 12700 Trajectory over Bathymetry','FontSize',20)

Bath = load(Meta.BathyFile);         %loads Bathymetry file fo2cruise specified above
...
for row = 1:1:length(d.data.lat)    %loop through the positions in the file
    ...
    %TExt= text(Longitude(row)+0.05,Latitude(row)+0.05,sprintf('%d',row),'FontSize',20)
end
...
%     %TExt= text(Longitude2(row)+0.05,Latitude2(row)+0.05,sprintf('%d',row),'FontSize',20)
% end
% 
Lat = d.data.lat;   %get Latitude variable
Lon = d.data.lon;  %get Longitude variable
FloSpecial = plot(Lon(17),Lat(17),'.','MarkerSize',2*Meta.MarkerSize,'LineWidth',7,'MarkerEdgeColor',[0.45 0 0]);
% 
% Cruise2 = line(Longitude2,Latitude2,'LineWidth',2,'Color',[0.75 0 0]);

legend([Flo FloSpecial],'12700 Float Traj','Profile 17 of 12700','location','NorthEast')

...
for i = 1:size(Data.data.T,1)
    plot((Data.data.T(i,:)+(i+13)),Data.data.P(i,:),'b')    %adds 15 to the index each time, ends up being a good way to increment
end 
...
xlabel('Specific Salinity (psu) + (index/5)')      % Using practical salinity with an eyeballed delta SP of 15
ylabel('In-situ Pressure')
...
for i = 1:size(Data.data.SP,1)
    plot((Data.data.SP(i,:)+(i/5)),Data.data.P(i,:),'b')
end 
